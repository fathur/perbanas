...
public static function wpcf_access_admin_edit_access($enabled = true)
{
    ...
    if (!empty($types))
    {
        $output .= '<h3>' . __('Post Types', 'wpcf_access') . '</h3>';
        foreach ($types as $type_slug => $type_data) {
			...
			$the_query = new WP_Query( $args );
				...
				if ( $type_slug == 'attachment' ){
					...
					__( 'This section controls acccess to media-element pages and not to media that is included in posts and pages.', 'wpcf_access' )
					. '</div>';
				}
    ...
    if (!empty($taxonomies))
    {
        $output .= '<h3>' . __('Taxonomies', 'wpcf_access') . '</h3>';
        foreach ($taxonomies as $tax_slug => $tax_data)
        {
            $mode = 'not_managed';
...
public static function wpcf_access_suggest_user($enabled = true, $managed = false)
{
    ...
    $output = '<div class="types-suggest-user types-suggest" id="types-suggest-user-' . $_id . '">';
		...
		$output .= '<select size="' . count($users) . '" class="dropdown">';
		foreach ($users as $u) {
