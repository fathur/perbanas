...
final class Access_Helper
{
	...
	*/
	public static function wpcf_access_create_shortcode_toolset_access( $atts, $content){
		...
		if ( in_array($current_role, $recived_roles_fixed) ){
			if ( $operator == 'allow' ){
				return self::wpcf_access_do_shortcode_content( $content);	
		 	}		
		}else{
			if ( $operator == 'deny' ){
				return self::wpcf_access_do_shortcode_content( $content);	
	...
	*/
	public static function wpcf_access_add_editor_icon( ){
		...
		$out = '<img src="'.TACCESS_ASSETS_URL . '/images/access-icon-16x16-gray.png" class="js-wpcf-access-editor-button" />';
		
		$out .= '<div class="editor_addon_dropdown js-wpcf-access-editor-popup" id="editor_addon_dropdown_' . md5( serialize( debug_backtrace() ) ) . '">
		...
		$out .= '<label>
        		<input type="checkbox" class="js-wpcf-access-list-roles" value="Guest" /> Guest</label><br>';	
		$out .= '</p>';
		...
		$out .= '<p class="wpcf-access-margin">
        	...
        	<label>
        		<input type="radio" class="js-wpcf-access-shortcode-operator" name="wpcf-access-shortcode-operator" value="deny" /> '. __('Everyone except these will see the text', 'wpcf_access') . '</label><br>
		</p>';
        $out .= '        
        </div>
	 ...
	 * 
	*/
	public static function wpcf_access_do_shortcode_content( $content ) 
    {
	...
	public static function wpcf_access_select_group_metabox_files( ) 
    {
		...
		if ( isset($post) && is_object($post) && $post->ID != '' ){
			...
			if ( !wp_script_is('icl_editor-script', 'registered')  ){
				wp_register_script('icl_editor-script', TACCESS_ASSETS_URL.'/js/icl_editor_addon_plugin.js', array(), WPCF_ACCESS_VERSION, false);
			}
...
//function wpcf_access_role_has_cap_filter($capabilities, $cap, $role) {}
    ...
    {
        global $wpcf_access, $wp_post_types, $user_level;
		$current_user = wp_get_current_user();
		if ($role == 'guest'){
			$user_level = 0;	
		} 
